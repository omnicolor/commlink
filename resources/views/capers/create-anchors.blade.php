<x-app>
    ...
    <div class="row">
        ...
        <div class="col">
            <select class="form-control" id="identity" name="identity" required>
                ...
                @foreach (\App\Models\Capers\Identity::all() as $option)
        ...
        <div class="col" id="identity-description">
            @if ('' !== $identity)
                {{ (new \App\Models\Capers\Identity($identity))->description }}
            @endif
        </div>
        <div class="col-1"></div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col">
            <select class="form-control" id="virtue" name="virtue" required>
                ...
                @foreach (\App\Models\Capers\Virtue::all() as $option)
                    <option
    ...
    <div class="mb-4 row">
        ...
        <div class="col">
            <select class="form-control" id="vice" name="vice" required>
                ...
                @foreach (\App\Models\Capers\Vice::all() as $option)
                    <option
    ...
    <x-slot name="javascript">
        <script>
            const suits = ['♣', '♦', '♥', '♠'];
            const identities = {
                @foreach (\App\Models\Capers\Identity::all() as $identity)
                    '{{ $identity->id }}': '{{ $identity->description }}',
                @endforeach
                '': ''
            };
            const virtues = {
                @foreach (\App\Models\Capers\Virtue::all() as $virtue)
                    '{{ $virtue->id }}': '{{ $virtue->description }}',
                @endforeach
                '': ''
            };
            const vices = {
                @foreach (\App\Models\Capers\Vice::all() as $vice)
